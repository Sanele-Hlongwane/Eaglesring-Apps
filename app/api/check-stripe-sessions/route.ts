...
export async function POST(req: Request) {
  try {
    ...
    for (const session of successfulSessions) {
      ...
      if (
        metadata &&
        metadata.entrepreneurProfileId &&
        metadata.investmentOpportunityId &&
        metadata.pitchTitle
      ) {
          ...
          await db.investment.create({
            data: {
              ...
              investmentOpportunity: {
                connect: { id: investmentOpportunityId },
              },
            },
          });
        }
    ...
    return new Response(JSON.stringify({ successfulSessions }), {
      status: 200,
    });
  } catch (error) {
    console.error("Error processing Stripe sessions:", error);
    return new Response(
      JSON.stringify({ error: "Error processing Stripe sessions" }),
      { status: 500 },
    );
  }
}
