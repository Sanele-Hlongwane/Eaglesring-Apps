...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    console.debug(`Notification sent to Sender ID ${senderId}: "${receiver.name} has rejected your friend request."`);

    return NextResponse.json({ success: true, updatedFriendRequest });

  } catch (error) {
    console.error('Error rejecting friend request:', error);
    return NextResponse.json({ error: 'Failed to reject friend request', details: (error as any).message }, { status: 500 });
  }
}
